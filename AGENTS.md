...
- `config/jetbrains-settings/` holds backup-only captures of portable JetBrains
  IDE settings; cloud Backup and Sync remains the live mechanism.
...
- Capture only portable JetBrains IDE settings (keymaps, colour/code styles,
  templates, allow-listed `options/*.xml`) as backup-only `jetbrains-app`
  manifest entries. Never capture license `*.key` files, `dataSources*`, `ssl/`,
...
- Extend Zirconium's existing chezmoi source and services; do not add another
  dotfile updater.
- JetBrains settings are backup-only: `jetbrains-app` entries capture the newest
  installed product directory (version-globbed live path) into version-
  normalised `config/jetbrains-settings/`, and chezmoi never deploys them. Cloud
  Backup and Sync owns live restore. `just validate` enforces the allow-list.
