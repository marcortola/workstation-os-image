...
- `rootfs/` — the OS image payload; the single tree baked into `/`
  (`COPY rootfs/ /`).
...
- `tooling/` — host-side management scripts, plus `tooling/ai/` (the AI-CLI
  machinery: the `ai-cli-setup` bundle and the install/build/reset scripts) and
  `tooling/fixtures/` (test data).

## Sources of truth

- `Containerfile` and `rootfs/` own OS packages, services, and host
  integration.
- `config/dotfiles.manifest` is the only inventory of personal configuration
  captured from the live account.
- `rootfs/usr/share/zirconium/zdots/` contains personal chezmoi seeds.
...
- Neovim is the in-repo IDE (LazyVim seeds under `dot_config/nvim/`). Language
  ...
  in nvim `lua/config/lazy.lua` imports only those extras (plus the universal
  ...
  language server must gate on nvim `lua/config/scope.lua`. Never re-add ungated or
...
- The two mixed AI-CLI configs (codex `config.toml`, claude `settings.json`) are
  ...
  cannot reinject codex trust/runtime tables beyond the allowlisted Help Scout
  and official OpenAI developer-docs MCP tables, the claude `env`/plugin blocks,
  or the rtk hook. Never revert them to plain `copy`. `just ai-reset` restores
  live codex/claude/opencode personal config to canonical, merging over
  preserved machine state (trust grants, keys); it never touches auth stores or
  histories.
- Third-party AI tools are never vendored. `just ai-tools-install` runs the
  official installers for caveman and opencode-fusion and places the
  playwright-cli skill, once after deploy like `just brew-apply`; rtk's binary
  comes from the Brewfile (`just brew-apply`) and the script only runs `rtk
  init`; the Help Scout MCP package is never installed, the launcher runs it via
  `npx` on demand. opencode.json is fusion-generated;
  our context7 fragment (`config/ai-tools/opencode-mcp-fragment.json`) and the
  secret-free Codex Help Scout MCP launcher/config are tracked. Post-install,
  rtk injects a hook into settings.json (scrubbed from the seed) and `@RTK.md`
  into CLAUDE.md/codex `AGENTS.md` (stripped from the seeds), so that live drift
  is expected, not captured. The image ships only our own browser and MCP glue:
  `playwright-cli`, `workstation-playwright-chrome`, and the Codex Help Scout
  launcher.
- `tooling/ai/ai-cli-setup/` is a portable, secret-free projection of the personal
  seeds for non-image installs; its `install.sh` runs the same official
  installers. `config/` + `opencode-mcp-fragment.json` are generated by `just
  ai-bundle` and checked by `just validate` — regenerate after changing seeds.
- Worktree untracked-file propagation is driven by each repo's committed
  ...
  `post_create`, `ga`, and the `init.templateDir` `post-checkout` hook go through
  `workstation-worktree-sync`; Claude Code's `--worktree` reads it natively; the
  JetBrains External Tool calls `git worktreeinclude` (Brewfile-installed) direct.
  Do not add a second copy list such as workmux `files.copy`.
