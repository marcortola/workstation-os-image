...
- `config/image-provided-brew-formulae` lists Homebrew shadows the image owns.
- `config/jetbrains-settings/_shared/` is the canonical "feel the same" JetBrains
  config; each `config/jetbrains-settings/<Product>/` holds only that IDE's
...
- The chezmoi source (`rootfs/usr/share/workstation-os-image/dotfiles/`) and the
...
- Never hand-edit generated output: manifest-captured seeds under
  `rootfs/usr/share/workstation-os-image/dotfiles/` (overwritten wholesale by
...
- niri resolves duplicate binds last-definition-wins and `config.kdl` includes
  ...
  it from `rootfs/usr/share/workstation-os-image/niri/includes/binds.kdl`; or
...
- Declare shared plugins as Marketplace IDs in
  `config/jetbrains-settings/_shared/plugins.list`; JARs are fetched at apply
...
- `opencode.json` is fusion-generated and never captured. Only our context7
  fragment (`config/ai-tools/opencode-mcp-fragment.json`) and the secret-free
...
- Fix the input, never the check. Never weaken, delete, or add an exception to a
  `tooling/validate` assertion or a `tooling/scrub/` allowlist to make a run
