...
- `Containerfile` and `rootfs/` own OS packages, services, host integration.
- `config/dotfiles.manifest` is the only inventory of captured personal config.
- `rootfs/usr/share/workstation-os-image/dotfiles/` holds the personal chezmoi seeds.
...
- Upstream-managed: Zirconium-owned Niri/DMS/GTK scaffolding. Personal Niri
  changes belong only in `local.kdl`.
...
- Extend Zirconium's chezmoi source and services; never add a second dotfile
  updater.
- Never hand-edit generated output: seeds under
  `rootfs/usr/share/workstation-os-image/dotfiles/` (overwritten wholesale by `just sync`) and
  `tooling/ai/ai-cli-setup/` (regenerated by `just ai-bundle`). Change the live
...
- Docker stays rootful and usable without `sudo` through dynamic local user
  group provisioning.
- Never add btop, chezmoi, Git, fzf, or Just to the Brewfile; Zirconium ships
  them. Never install Fish through Homebrew; the image owns `/usr/bin/fish`.
...
- Tracked items whose live value drifted from their captured baseline. Confirm
  ...
  baseline (`dms-capture --list` shows live vs Zirconium default, hiding
  live-vs-overlay drift). Never dismiss this as expected.
