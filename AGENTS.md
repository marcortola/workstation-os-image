...
- The two mixed AI-CLI configs (codex `config.toml`, claude `settings.json`) are
  ...
  histories.
- Third-party AI tools (opencode-fusion, caveman, rtk, playwright-cli, the Help
  Scout MCP npm package) are NOT
  vendored: `just ai-tools-install` installs them via their official installers,
  once after deploy like `just brew-apply`. opencode.json is fusion-generated;
...
- `tooling/ai/ai-cli-setup/` is a portable, secret-free projection of the personal
  ...
  ai-bundle`, validated in sync -- regenerate after changing seeds, never hand-edit.
