...
- The two mixed AI-CLI configs (codex `config.toml`, claude `settings.json`) are
  ...
  cannot reinject the codex trust tables, the claude `env`/plugin blocks, or the
  rtk hook. Never revert them to plain `copy`. `just ai-reset` restores live
  codex/claude/opencode personal config to canonical, merging over preserved
  machine state (trust grants, keys); it never touches auth stores or histories.
- Third-party AI tools (opencode-fusion, caveman, rtk, playwright-cli) are NOT
  ...
  only our context7 fragment (`config/ai-tools/opencode-mcp-fragment.json`) is
  tracked. Post-install, rtk injects a hook into settings.json (scrubbed from the
  seed) and `@RTK.md` into CLAUDE.md/codex `AGENTS.md` (stripped from the seeds),
  so that live drift is expected, not captured. The image ships only our own
  browser glue: `playwright-cli` wrapper + `workstation-playwright-chrome`.
