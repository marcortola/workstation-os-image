...
- `image/build/` owns OS packages, repositories and build steps;
  `image/rootfs/` owns services and host integration.
...
- `image/rootfs/usr/share/workstation-os-image/` holds the chezmoi seeds under
...
- The chezmoi source (`image/rootfs/usr/share/workstation-os-image/dotfiles/`) and the
...
- Never hand-edit generated output: manifest-captured seeds under
  `image/rootfs/usr/share/workstation-os-image/dotfiles/` (overwritten wholesale by
...
- niri resolves duplicate binds last-definition-wins and `config.kdl` includes
  ...
  it from `image/rootfs/usr/share/workstation-os-image/niri/includes/binds.kdl`; or
