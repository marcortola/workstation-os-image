...
| [herdr-oom-blast-radius.md](herdr-oom-blast-radius.md) | Why one out-of-memory kill anywhere on the machine SIGKILLed the work running in every terminal pane, why the workspace layout survived it and the running work did not, why the fix is two directives because `OOMPolicy` cannot prevent a systemd-oomd kill — only decide the state after one — and the six routes that lost — `omit`, a manager-wide `DefaultOOMPolicy`, a `service.d` drop-in that overrides the units it was meant to help, a transient scope for the browser, a fix scoped to the playwright helper, and waiting for the image to catch up. |
...
| [scaffold-force-apply.md](scaffold-force-apply.md) | Why the dotfile updater now overwrites image-owned targets instead of preserving them, how one skipped file left a herdr plugin frozen for three days while reporting success, why forcing the whole `scaffold` kind is safe when two of its entries are create-only, and the five routes that lost — forcing everything, a hand-written list, resetting chezmoi's entry state, symlinking from `/usr/share`, and auditing without repairing. |
| [checkout-removal.md](checkout-removal.md) | Why `git worktree remove` left six checkouts on disk that git no longer knew about, why the `--force` retry that followed could never work, why the popup now deletes a merged branch it used to always keep, and the seven routes that lost — fixing the containers, refusing with homework, retrying with `--force`, calling `/worktree-remove`, deleting the branch last, forcing an unmerged branch, and checking merge state without the network. |
| [playwright-npm-cache-and-browser-teardown.md](playwright-npm-cache-and-browser-teardown.md) | Why one `playwright-cli` subcommand cost 51 s of pinned CPU, why the `@latest` dist-tag everyone blames was the trigger and not the cause, why the desktop freeze was neither, and the eight routes that lost — pinning the version, baking the CLI into the layer, `cache=~/.npm`, `environment.d`, `readlink -f`, a chezmoi seed for `~/.npmrc` that worked and was reverted because npm writes credentials into that file, killing `flatpak run`'s pid, and `--die-with-parent`, which passes its own A/B test and then kills the browser the moment the script exits. |
| [multi-agent-checkouts.md](multi-agent-checkouts.md) | Why a checkout can now run claude, codex and opencode side by side, why the slot is named after the agent rather than numbered, why the recency stamp had to grow a second key before any of it was safe, and the four routes that lost — numbered slots, a slot-to-agent state file, folding a second agent into the split tab, and per-agent rows in the space picker. |
| [dms-greeter-split.md](dms-greeter-split.md) | Why every image build failed for four days against a gate that was correct when written, how `dms-greeter` leaving DankMaterialShell for an upstream project of its own invalidated its premise five days later, and the five routes that lost — pinning to a NEVRA COPR prunes in fourteen days, sourcing both from one COPR, dropping the greeter check into a tautology, waiting for a publish that was never coming, and warning instead of failing. |
