...
| `sources` | `CLAUDE.md` is exactly `@AGENTS.md`; `AGENTS.md` stays under its 398-line cap and `README.md` under its 170-line cap; at least 15 pages exist under `docs/` and at least one agent command under `.claude/commands/`; every `just`/`wjust` recipe and every `system_files/`, `build_files/`, `tooling/` or `docs/` path cited in `AGENTS.md`, `README.md`, every `docs/**/*.md` and `.claude/commands/*.md` exists; every relative link in those same files resolves, a same-file anchor matches a real heading, and a cross-file anchor is rejected outright; the AI seeds carry no key, plugin identifier or absolute home path; the worktree wiring is intact; the herdr server unit keeps `KillMode=mixed`, `OOMPolicy=continue`, `ManagedOOMPreference=avoid`, an `ExecStart` under `/usr/libexec` and its `ExecCondition`, both session launch paths still start it, no expiry constant returns to the dev-flow plugin, and the split layout's `dev` tab mark and the startup status reset stay wired; every tracked JSON, TOML and YAML parses; shipped scripts use one of two shebangs | CI `repo-gates` and local | policy prose rots into dangling pointers, the tool-specific secret shapes gitleaks misses go uncaught, and a dropped `KillMode` costs a workspace at the next reboot with nothing else to show for it |
...
| `source-images` | every digest-pinned reference in the Containerfile is cosign-verified against a vendored key, default-deny by registry prefix, and the digest is still fetchable. Only `manifest unknown` counts as pruned; a rate limit, 5xx or DNS failure is reported as unanswered rather than as a dead pin, and an unanswered run does not print the clean-bill line | CI `repo-gates` and local, guarded on `command -v cosign` | a compromised upstream push lands on the machine unremarked. See [supply-chain.md](supply-chain.md) |
...
| `keybindings/build-cheatsheet.py --check` | not under `validate/`, but driven from `all`: regenerates the `Mod+Slash` cheatsheet and diffs it against the committed seed. Separately asserts that every niri bind without a `hotkey-overlay-title` is named in `tooling/data/niri-bind-descriptions`, that no entry there names a bind that no longer exists, and that every `##` section of `docs/keybindings.md` is placed | local only, driven from `all` | a new bind drops out of the cheatsheet silently, or the sheet drifts from the doc it is generated from |
...
| `units` | every `enable`/`disable` line in both preset files against live `systemctl is-enabled`; any failed unit that is image-owned; and, for image-owned units whose failed state has been cleared, an `ExecMainStatus` that says the last run exited non-zero and nothing has succeeded since. Also that a running herdr server is inside `workstation-herdr-server.service` rather than in the cgroup of the terminal that launched it, and that the live unit really carries `OOMPolicy=continue` and `ManagedOOMPreference=avoid` — a booted deployment can lag the repository | a local `systemctl mask` survives every upgrade with the build-time gate still green, a `systemctl reset-failed` turns a unit that never worked into a clean audit, and a herdr started outside its unit loses a workspace at the next logout with no other signal |
...
| `dms-settings` | the captured overlay applied to a copy of live settings reproduces live settings once overlay entries that merely restate a DMS default are dropped -- DMS never stores those, so they are reported separately as dead pins rather than counted as drift -- and `tooling/dms/capture --list` reports no `[new]` portable deviation | reviewed GUI preferences never become workstation defaults. See [subsystems/desktop-session.md](subsystems/desktop-session.md) |
...
same commit — the 398-line cap on `AGENTS.md` in `tooling/validate/sources`
