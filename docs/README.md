...
| Development environment | [subsystems/dev-environment.md](subsystems/dev-environment.md) | No global runtimes; the `dev` wrapper and the `pro` project picker; `dev nvim` running Neovim inside the container, its per-project store and language scoping; why the IDEs keep their own keymap; worktree file propagation; herdr. |
