...
| 7 | [capturing-changes.md](capturing-changes.md) | The audit → capture → sync → validate loop, the dotfile manifest and its capture kinds, the DMS preference overlay, and how to answer "is there drift?" honestly. |
...
| Development environment | [subsystems/dev-environment.md](subsystems/dev-environment.md) | No global runtimes; the `dev` wrapper and the `pro` project picker; `dev nvim` running Neovim inside the container, its per-project store and language scoping; worktree file propagation; herdr. |
| Agent parked-work probes | [subsystems/agent-probes.md](subsystems/agent-probes.md) | Why a checkout whose agent left a background task running reads as idle, the per-agent probe contract behind `/usr/libexec/workstation-agent-probes/`, which agents have a signal and why the rest do not, and the process-tree predicate that was measured false. |
