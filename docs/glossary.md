...
| **dotfiles manifest** | `tooling/data/dotfiles.manifest`, the only inventory of captured personal configuration. Each line is `kind`, live path relative to `$HOME`, chezmoi source path and file pattern, separated by pipes; the kinds are `copy`, `template`, `scrub`, `scaffold`, `directory` and `tree`. |
...
| **`base-digest.yml` and Dependabot** | Both bump pinned inputs, and the split is deliberate. `.github/workflows/base-digest.yml` maintains the `BASE_IMAGE` ARG; Dependabot's Docker parser only reads literal `FROM` lines, so it maintains the brew stage's digest — which is why the brew image is a named build stage rather than a `COPY --from=<image>`. Renovate owned the ARG until 2026-09-16, when it emerged that the App had never been installed and the pin had never moved on its own. |
