...
| Path | Why |
|---|---|
| `image.env` | Image name, registry owner, description — `IMAGE_NAME`, `REPO_ORGANIZATION`, `IMAGE_DESC`. The one place identity is written. The Justfile reads it as its dotenv file (`set dotenv-filename := "image.env"`), `tooling/audit/deployment` builds `expected_image="ghcr.io/${REPO_ORGANIZATION}/${IMAGE_NAME}:"` from it, `build_files/40-signing.sh` derives the signing policy scope `scope="ghcr.io/${REPO_ORGANIZATION}"`, `system_files/usr/bin/wjust` clones `https://github.com/${REPO_ORGANIZATION}/${IMAGE_NAME}.git`, and the Containerfile interpolates all three into the OCI labels. It is also `COPY`d to `/usr/share/workstation-os-image/image.env` so the running machine resolves the same values. |
...
| `tooling/data/jetbrains-settings/` and `tooling/jetbrains/` | Repo-owned JetBrains configuration plus the seven scripts that diff, promote and apply it. | The settings are personal; the machinery is generic, so the usual move is to keep `tooling/jetbrains/` and empty the data. Deleting the machinery too means dropping the `jetbrains-*`, `ide-setup` and `intelephense-licence` recipes from the Justfile and the `tooling/jetbrains/validate` line from `tooling/validate/all`. The workflow it implements is described in [capturing-changes.md](capturing-changes.md). |
