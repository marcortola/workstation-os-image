#!/usr/bin/bash
# Build-time gates. Mutates nothing that survives them -- the DMS policy probe
# creates and removes /run/ostree-booted, because dms consults the policy only
# on a system it judges immutable.
#
...
# The refusal, unconditionally: `strings -a /usr/bin/dms-greeter` carries
# `dms-greeter uninstall` and `=== DMS Greeter Uninstall ===` but no
# `Unknown option`, so an absent-subcommand alternative would match nothing and
# weaken the assertion to a tautology.
grep -F 'Detected immutable system' >/dev/null \
    <<<"$dms_greeter_uninstall" \
    || fail "dms-greeter uninstall is not blocked by the CLI policy"
